...
def list_usuarios_route(db: Session = Depends(get_db), current_user=Depends(get_current_usuario)):
    try:
...
def view_usuario_route(id_usuario: int, db: Session = Depends(get_db), current_user=Depends(get_current_usuario)):
    try:
...
def update_usuario_route(id_usuario: int, usuario: UsuarioUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_usuario)):
    try:
        return update_usuario(db, id_usuario, usuario)
    except Exception as e:
        logger.error(f"Erro ao atualizar o Usuário: {str(e)}")
        raise HTTPException(
            status_code=500, detail="Erro ao atualizar o Usuário")


@usuario_router.delete("/delete/{id_usuario}", response_model=UsuarioBase)
def delete_usuario_route(id_usuario: int, db: Session = Depends(get_db), current_user=Depends(get_current_usuario)):
    try:
        return delete_usuario(db, id_usuario)
    except Exception as e:
        logger.error(f"Erro ao deletar o Usuário: {str(e)}")
        raise HTTPException(
            status_code=500, detail="Erro ao deletar o Usuário")
