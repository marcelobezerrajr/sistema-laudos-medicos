...
def list_exames_route(db: Session = Depends(get_db)):
    try:
...
def view_exame_route(id_exame: int, db: Session = Depends(get_db)):
    try:
...
def add_exame_route(exame: ExameCreate, db: Session = Depends(get_db)):
    try:
...
def update_exame_route(id_exame: int, exame: ExameUpdate, db: Session = Depends(get_db)):
    try:
...
def delete_exame_route(id_exame: int, db: Session = Depends(get_db)):
    try:
