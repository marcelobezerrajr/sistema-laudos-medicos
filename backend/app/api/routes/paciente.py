...
def view_paciente_route(id_paciente: int, db: Session = Depends(get_db)):
    try:
        return get_paciente_by_id(db, id_paciente)
    except Exception as e:
        logger.error(f"Erro ao listar o Paciente: {str(e)}")
        raise HTTPException(status_code=500, detail="Erro ao listar o Paciente")

@paciente_router.post("/create", response_model=PacienteOut)
def add_paciente_route(paciente: PacienteCreate, db: Session = Depends(get_db)):
    try:
...
def update_paciente_route(id_paciente: int, paciente: PacienteUpdate, db: Session = Depends(get_db)):
    try:
        return update_paciente(db, id_paciente, paciente)
    except Exception as e:
        logger.error(f"Erro ao atualizar o Paciente: {str(e)}")
        raise HTTPException(status_code=500, detail="Erro ao atualizar o Paciente")

@paciente_router.delete("/delete/{id_paciente}", response_model=PacienteOut)
def delete_paciente_route(id_paciente: int, db: Session = Depends(get_db)):
    try:
        return delete_paciente(db, id_paciente)
    except Exception as e:
        logger.error(f"Erro ao deletar o Paciente: {str(e)}")
        raise HTTPException(status_code=500, detail="Erro ao deletar o Paciente")
