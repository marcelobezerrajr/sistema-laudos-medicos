...
def list_laudos_route(db: Session = Depends(get_db)):
    try:
...
def view_laudo_route(id_laudo: int, db: Session = Depends(get_db)):
    try:
...
def add_laudo_route(laudo: LaudoCreate, db: Session = Depends(get_db)):
    try:
...
def update_laudo_route(id_laudo: int, laudo: LaudoUpdate, db: Session = Depends(get_db)):
    try:
...
def delete_laudo_route(id_laudo: int, db: Session = Depends(get_db)):
    try:
