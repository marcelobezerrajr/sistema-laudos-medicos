...
class Medico(Base):
    ...
    id_medico = Column(
        Integer, ForeignKey("usuarios.id_usuario", ondelete="CASCADE"), primary_key=True
    )
...
class Exame(Base):
    ...
    id_paciente = Column(
        Integer, ForeignKey("pacientes.id_paciente", ondelete="CASCADE"), nullable=False
    )
    tipo_exame = Column(String(255), nullable=False)
    data_envio = Column(TIMESTAMP, default=func.now(), nullable=False)
    status = Column(
        SqlEnum(Status_Exame), default=Status_Exame.pendente, nullable=False
    )
...
class ImagemExame(Base):
    ...
    id_exame = Column(
        Integer, ForeignKey("exames.id_exame", ondelete="CASCADE"), nullable=False
    )
...
class Laudo(Base):
    ...
    id_medico = Column(
        Integer, ForeignKey("medicos.id_medico", ondelete="CASCADE"), nullable=False
    )
...
class ImagemLaudo(Base):
    ...
    id_imagem = Column(Integer, primary_key=True, autoincrement=True)
    id_laudo = Column(
        Integer, ForeignKey("laudos.id_laudo", ondelete="CASCADE"), nullable=False
    )
    caminho_arquivo = Column(String(255), nullable=False)
    descricao = Column(String(255))
